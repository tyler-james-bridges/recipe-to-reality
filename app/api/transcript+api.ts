...
export async function POST(request: Request) {
  try {
    ...
    if (!url || !platform) {
      return Response.json({ error: 'Missing required fields: url, platform' }, { status: 400 })
    }
...
async function extractSupadataTranscript(urlString: string, apiKey: string): Promise<string> {
