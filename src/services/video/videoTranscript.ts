  ...
  switch (statusCode) {
    case 401:
      return {
        ...
        isRetryable: false,
        recoverySuggestion: 'Check your API key in Settings > Video Platforms and ensure it is correct.',
      };
    case 403:
      return {
    ...
    case 429:
      return {
        ...
        isRetryable: true,
        recoverySuggestion: 'You have exceeded your API rate limit. Please wait a few minutes and try again.',
      };
    case 500:
    case 502:
    case 503:
    case 504:
      return {
        ...
        isRetryable: true,
        recoverySuggestion: 'The transcript service is experiencing issues. Please try again in a few minutes.',
      };
    default:
      return {
        message: errorMessage || `Unexpected error (HTTP ${statusCode})`,
        isRetryable: statusCode >= 500,
        recoverySuggestion: 'An unexpected error occurred. Please try again or contact support if the issue persists.',
      };
  }
}
...
async function extractYouTubeTranscript(url: URL): Promise<string> {
  ...
  const operation = async (): Promise<string> => {
    ...
    // Fetch the captions
    const captionsResponse = await fetchWithTimeout(
      track.baseUrl,
      {},
      YOUTUBE_TIMEOUT_MS
    );
  ...
  try {
    ...
    throw new VideoTranscriptError(
      `Failed to extract transcript: ${errorMessage}`,
      'youtube',
      {
 ...
 */
export async function testSupadataAPIKey(apiKey: string): Promise<{ valid: boolean; error?: string }> {
  if (!validateApiKey(apiKey)) {
  ...
  try {
    ...
    return { valid: false, error: 'Could not validate API key. Please check your internet connection.' };
  }
}
