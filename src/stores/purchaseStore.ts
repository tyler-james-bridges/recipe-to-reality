...
export const usePurchaseStore = create<PurchaseState>((set, get) => ({
  ...
  initialize: async () => {
    try {
      ...
      if (!apiKey) {
        console.warn(
          'RevenueCat API key not configured. Set EXPO_PUBLIC_REVENUECAT_IOS_KEY or EXPO_PUBLIC_REVENUECAT_ANDROID_KEY in your .env file.'
        )
        return
      }
  ...
  purchase: async (pkg) => {
    if (!isRevenueCatAvailable || !Purchases) {
      throw new Error(
        'Purchases not available in Expo Go. Build a development client to test purchases.'
      )
    }
